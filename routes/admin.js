...
router.post('/complaints', async (req, res) => {
    ...
    try {
        ...
        // Save to the database
        const sql = 'INSERT INTO complaints (description, severity, status, created_at, department, year_level) VALUES (?, ?, ?, ?, ?, ?)';
        db.run(sql, [description, severity, complaintData.status, complaintData.created_at, complaintData.department, complaintData.year_level], function(err) {
            if (err) {
                console.error('Error saving complaint:', err);
                return res.status(500).json({ message: 'Error saving complaint' });
            }
            res.json({ message: 'Complaint submitted successfully', id: this.lastID, severity });
        });
    } catch (error) {
        console.error('Error analyzing severity:', error);
        res.status(500).json({ message: 'Error analyzing severity' });
...
router.get('/dashboard', isAuthenticated, (req, res) => {
    // Get department counts
    db.all('SELECT department, COUNT(*) AS count FROM complaints GROUP BY department', (err, departmentResults) => {
        if (err) return res.status(500).send('Error retrieving department counts');

        // Get status counts
        db.all('SELECT status, COUNT(*) AS count FROM complaints GROUP BY status', (err, statusResults) => {
            if (err) return res.status(500).send('Error retrieving status counts');

            // Get year level counts
            db.all('SELECT year_level, COUNT(*) AS count FROM complaints GROUP BY year_level', (err, yearLevelResults) => {
                if (err) return res.status(500).send('Error retrieving year level counts');

                // Get monthly trends
                db.all('SELECT strftime("%m", created_at) AS month, COUNT(*) AS count FROM complaints GROUP BY month', (err, monthlyResults) => {
                    if (err) return res.status(500).send('Error retrieving monthly trends');

                    // Get weekly trends
                    db.all('SELECT strftime("%W", created_at) AS week, COUNT(*) AS count FROM complaints GROUP BY week', (err, weeklyResults) => {
                        if (err) return res.status(500).send('Error retrieving weekly trends');
...
router.get('/dashboard/feedback', isAuthenticated, (req, res) => {
    // Query to get feedback
    db.all('SELECT * FROM feedback ORDER BY created_at DESC', (err, feedback) => {
        if (err) {
            console.error('Error retrieving feedback:', err);
            return res.status(500).send('Error retrieving feedback');
        }
        ...
        let departmentCounts = {};
        // Query to get department counts
        db.all('SELECT department, COUNT(*) as count FROM feedback GROUP BY department', (err, counts) => {
            if (err) {
                console.error('Error retrieving department counts:', err);
            } else {
                counts.forEach(row => {
        ...
        db.all('SELECT department, COUNT(*) AS count FROM complaints GROUP BY department', (err, departmentResults) => {
            ...
            db.all('SELECT year_level, COUNT(*) AS count FROM complaints GROUP BY year_level', (err, yearLevelResults) => {
                ...
                db.all('SELECT status, COUNT(*) AS count FROM complaints GROUP BY status', (err, statusResults) => {
                    ...
                    // Monthly Trends Query (example)
                    db.all('SELECT strftime("%m", created_at) AS month, COUNT(*) AS count FROM complaints GROUP BY month ORDER BY month ASC', (err, monthlyResults) => {
                        if (err) {
                            console.error('Error retrieving monthly trends:', err);
                            return res.status(500).send('Error retrieving monthly trends');
                        }
                        ...
                        // Weekly Trends Query (example)
                        db.all('SELECT strftime("%W", created_at) AS week, COUNT(*) AS count FROM complaints GROUP BY week ORDER BY week ASC', (err, weeklyResults) => {
                            if (err) {
                                console.error('Error retrieving weekly trends:', err);
                                return res.status(500).send('Error retrieving weekly trends');
                            }
